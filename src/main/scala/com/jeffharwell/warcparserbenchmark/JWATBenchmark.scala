...
import org.jwat.warc.{WarcReader, WarcReaderFactory, WarcRecord}
...
class JWATBenchmark(filename: String) {
  ...
  def runBenchmark(): Unit = {
    ...
    val reader: WarcReader = WarcReaderFactory.getReaderCompressed(new BufferedInputStream(
      new FileInputStream(new File(filename)))
    )
    val parser = reader.iterator()

    val warc_ids = parser.map{x => x.getHeader("WARC-Record-ID")}
